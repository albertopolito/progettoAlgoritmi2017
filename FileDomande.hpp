#ifndef FILEDOMANDE_HPP_INCLUDED
#define FILEDOMANDE_HPP_INCLUDED
#include"FileInput.h"
...
const bool FileDomande<T,R>:: leggiFile(const string& nome_file)
{
    ...
    //guardo che tutte le adiacenze siano verificate
    vector<T> domande_da_grafo=_grafo_domande_da_sottoporre.getTuttiIcontenuti();
    for(typename vector<T>::iterator it_v=domande_da_grafo.begin();it_v!=domande_da_grafo.end();it_v++)
    {
        if(_vocabolario_domande.getStringaDaId(*it_v)=="")
        {
            chiudiFileInput();
            return 1;
        }
    ...
    chiudiFileInput();
    //ritorna l'ultimo controllo, cioè le ripetizioni di domande o risposte
